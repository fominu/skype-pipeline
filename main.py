...
@app.get("/")
def health():
    return {"status": "ok"}  # healthcheck, Cloud Run увидит, что сервер жив
...
async def ingest(request: Request):
    ...
    if not bucket or not name or not name.lower().endswith(".mp4"):
        return {"status": "skipped", "reason": "no mp4 event"}
